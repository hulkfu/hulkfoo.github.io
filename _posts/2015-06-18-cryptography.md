...
nonce 是接收者先向发送者发送的，更安全，连用户自己也不能进行重放。典型应用有网络游戏的服务端，这样被抓包也不怕了，有密钥也不怕，反正 nonce 是 server 发送的。

有时序号和时间戳也是通过 nonce 这个名字传的，因为它的意思就是“现时，目前”。

# 数字签名 —— 消息到底是谁写的

逻辑上它和公钥密码相反，签名者用私钥签名，认证者用公钥认证。在公钥密码里，私钥是用来解密，而公钥是用来加密发送的消息的。这里的公是公开，私是私有，不管它们的功能是加密还是解密。
