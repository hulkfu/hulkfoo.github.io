...
# require vs load
require就是把所require的代码复制到当前require的位置，并执行一遍，让下面的代码使用其定义的方法、类等代码。使用require就是组织代码，让它们在不同的文件夹不同的文件里放着却可以逻辑上在一起，还有就是代码的复用。load是每次遇见都会执行。
...
* 默认第二个参数其实是"./"，本来expand_path方法的作用就是把相对路径转为绝对路径的。
* 常见的使用**__FILE__**的作用是不管执行程序的目录是哪个，都得到相对于代码所在位置的相对路径，所以在**require**里常见。
* 对内自己用的要用__FILE__来定位一下，对外的输入就要默认相对的就ok。

同理**require_relative**也是这个作用，根据当前文件目录，按照相对路径，寻找需要引用的文件。
而不是执行当前程序的目录。
